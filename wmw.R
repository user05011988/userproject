...
function(file) {
 ...
 file=pwdfile
 x <- read.csv(file, sep=",", header=TRUE,row.names=NULL)
 x.x = x[,3:ncol(x),drop=F]
 # rownames(x.x) = x[,2]
 k = matrix(x[,1], ncol=1)
 x.n = cbind(k, x.x)
...
#perform WMW test for the effective combinations and extract p-values
    ...
    for (q in 1:fin) {
    wilx.pv[q,] <- try(wilcox.test(I[,q],J[,q], paired=FALSE, exact=NULL, correct=FALSE, conf.level=0.95, alternative="two.sided")$p.value)
    }
