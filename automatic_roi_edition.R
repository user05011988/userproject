automatic_roi_edition = function(dataset,
                                 ...
                                 Xdata,
                                 program_parameters,ROI_limits,ppm) {
...
quartile_spectrum = as.numeric(apply(dataset[, program_parameters$ROI_buckets,drop=F], 2, function(x)
  quantile(x, 0.75)))
reference_spectrum_ind = which.min(apply(dataset[, program_parameters$ROI_buckets,drop=F], 1, function(x)
  sqrt(mean((x - quartile_spectrum) ^ 2
  ))))

#I localize the peaks of this reference ROI
peak_info = peakdet(dataset[reference_spectrum_ind, program_parameters$ROI_buckets,drop=F], 0.01)
...
for (i in 1:dim(import_excel_profile)[1]) {
  if (import_excel_profile[i, 8] == 1) {
    ...
    if (abs(round(reference_peaks_location[reference_peak_ind] - ROI_shift_location, 3)) <=
        round(program_parameters$automatic_shift_distance, 3))
    ...
    ROI_shift_location = c(Xdata[which.min(abs((
      import_excel_profile[i, 5] - import_excel_profile[i, 9] / (2 * program_parameters$freq)
    ) - Xdata
    ))],
    Xdata[which.min(abs((
      import_excel_profile[i, 5] + import_excel_profile[i, 9] / (2 * program_parameters$freq)
    ...
    #triplet
    ROI_shift_location = c(Xdata[which.min(abs((
      import_excel_profile[i, 5] - import_excel_profile[i, 9] / program_parameters$freq
    ) - Xdata
    ))],
    ...
    if (min(minimizer) <= round(program_parameters$automatic_shift_distance, 3))
  ...
  # 
  # fa=approx(Xdata,dataset[reference_spectrum_ind, program_parameters$ROI_buckets,drop=F],gg)
  ...
  # cccc[1]=cccc[1]+1
  # import_excel_profile[i, 6]=program_parameters$freq*(fa$x[aa[cccc[2]]]-fa$x[aa[cccc[1]]])/2
...
spectra_lag = matrix(NA, dim(import_excel_profile)[1],dim(dataset[, program_parameters$ROI_buckets,drop=F])[1])
for (i in 1:dim(dataset[, program_parameters$ROI_buckets,drop=F])[1]) {
  ...
  # spectra_lag[j,i] = d$lag[which.max(d$acf)]
  # }
  import_excel_profile[j, 11] = (max(abs(spectra_lag[j,])) * program_parameters$buck_step) +
    0.002
  # }
}
