peak_analysis=function(dataset,ppm,freq,export_path,metadata,repository) {
  ...
  peakParam$ampThr=quantile(dataset,0.60,na.rm=T)
...
for (j in 1:dim(peak_info)[1]) {
  ...
  peak_shape_corr[,j]=tryCatch(suppressWarnings(cor(t(med2),med)),error=function(e) NaN)
}
peak_shape_corr[is.na(peak_shape_corr)]=0
colnames(peak_shape_corr)=peak_ppm

valid_peak_ind=apply(peak_shape_corr,2,function(x)quantile(x,0.4))
...
corr_matrix=round(suppressWarnings(cor(peak_quantification,use='pairwise.complete.obs',method='spearman')),2)

threshold_corr_matrix=corr_matrix
threshold_corr_matrix[threshold_corr_matrix<0.8]=0

correlated_signals_indexes=threshold_corr_matrix[which(colSums(threshold_corr_matrix,na.rm=T)>1),which(colSums(threshold_corr_matrix,na.rm=T)>1)]
...
clustering_corr_matrix=corr_matrix[which(colSums(threshold_corr_matrix,na.rm=T)>1),which(colSums(threshold_corr_matrix,na.rm=T)>1)]
clustering_corr_matrix=clustering_corr_matrix[-ab2,-ab2]
signals_clusters=list()
if (nrow(clustering_corr_matrix)>0) {
...
signals_list=unique(append(secure_multiplets,signals_clusters))
  ...
  signals_list=secure_multiplets
}
...
# suggested_metabolites[i,]=paste(repository[!is.na(repository[,3]),][sort(abs(repository[,3]-ROI_profile_suggestion[i,5]),index.return=T)$ix[1:5],1],' (', round(sort(abs(repository[,3]-ROI_profile_suggestion[i,5]),index.return=T)$x[1:5],3),')',sep='')
