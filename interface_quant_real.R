    ...
    # print(ROI_profile)
      ...
      for (i in 1:dim(ROI_profile)[1]) {
        ...
        output_data$signals
            ...
            plot_data=matrix(0,dim(dummy3)[1],dim(autorun_data$dataset)[2])
            plot_data[,ROI_buckets]=dummy3

            for (i in 1:length(dummy2)) {
              if (length(dummy2[[i]])>0) {
                print('Ha')
                dummy4=ROI_buckets+(ROI_profile[i,5]-dummy[dummy2[[i]],5])/autorun_data$buck_step
                plot_data[c(1,3),dummy4]=plot_data[c(1,3),ROI_buckets]

                plot_data=rbind(plot_data,rep(0,dim(autorun_data$dataset)[2]))

                plot_data[dim(plot_data)[1],dummy4]=plot_data[3+i,ROI_buckets]
      ...
      plotdata2 = data.frame(Xdata= autorun_data$ppm,
        Ydata= as.numeric(autorun_data$dataset[spectrum_index, ]),
        plot_data[3, ],
        plot_data[2, ] )
     ...
     #  print(dim(plotdata5))
      ...
      plotdata = data.frame(Xdata, signals = plot_data[1, ] )
      p=plot_ly(plotdata,x = ~Xdata, y = ~signals, type = 'scatter', name= autorun_data$signals_names,mode = 'lines', fill = 'tozeroy') %>% add_trace(data=plotdata3,x=~Xdata,y=~value,color=~variable,type='scatter',mode='lines',fill=NULL)  %>% add_trace(data=plotdata5,x=~Xdata,y=~value,color=~'Surrounding signals',type='scatter',mode='lines',fill=NULL)  %>%
