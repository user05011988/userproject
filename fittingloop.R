fittingloop = function(FeaturesMatrix,
  ...
  while (iterrep < fitting_maxiterrep) {
    ...
    #while (error1 > 3 &
      ...
      if (iter<2) {
      lol = peakdet(Ydata, other_fit_parameters$peakdet_minimum*0.1*max(1e-10,max(Ydata)),Xdata)
    ...
    if (error2 < (other_fit_parameters$additional_signal_improvement * dummy) &
      ...
      lol = peakdet(nls.out$fvec, other_fit_parameters$peakdet_minimum*max(1e-10,max(Ydata)))
