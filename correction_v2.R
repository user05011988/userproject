correction_v2 = function(finaloutput,autorun_data) {
...
for (ii in 1:dim(shift_corrmatrix)[1]) {
  ...
  rlm_samples_sub[finaloutput$fitting_error[,shift_reference[[ii]]]>5,2]=NA
  rlm_samples_sub[finaloutput$signal_area_ratio[,shift_reference[[ii]]]<10,2]=NA
  #Robust linear model and prediction of expected shifts
  rlm_model=lmrob(rlm_samples_sub[,1]~rlm_samples_sub[,2])
  shift_prediction[,ii]=as.numeric(rlm_model$coefficients[1])+as.numeric(rlm_model$coefficients[2])*rlm_samples_sub[,2]
  shift_prediction[is.na(shift_prediction[,ii]),ii]=finaloutput$shift[is.na(shift_prediction[,ii]),ii]
