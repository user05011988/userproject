...
observeEvent(input$autorun, {
  ...
  output$plot <- renderPlotly({
    if (reactivequantdata$stop3==0&length(reactiveprogramdata$info)==0|reactivequantdata$stop3==0&length(input$x1_rows_selected)>1) {
      ROI_limits=c(which(round(reactiveprogramdata$autorun_data$ppm,6)==round(reactiveprogramdata$ROIdata[1,1],6)),
      which(round(reactiveprogramdata$autorun_data$ppm,6)==round(reactiveprogramdata$ROIdata[1,2],6)))
